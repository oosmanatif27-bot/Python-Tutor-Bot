...
lessons_py = {
    ...
    "4": {"title": "الدرس 4: دالة input 💡", "explanation": "استقبال البيانات من المستخدم.", "example": "age = input('Enter your age: ')", "exercise": "اطلب من المستخدم إدخال لونه المفضل.", "solution": "color = input('What is your favorite color? ')"},
    "5": {"title": "الدرس 5: القوائم Lists 💡", "explanation": "تخزين عدة قيم في متغير واحد.", "example": "tools = ['Python', 'C++', 'Java']", "exercise": "أنشئ قائمة فيها الأرقام 1 و 2 و 3.", "solution": "numbers = [1, 2, 3]"},
...
lessons_cpp = {
    ...
    "4": {"title": "➗ 4: العمليات الحسابية", "explanation": "نفس بايثون مع أهمية الفاصلة المنقوطة ;", "example": "int x = 5 + 5;", "exercise": "اضرب 5 في 10 وخزنها في x.", "solution": "int x = 5 * 10;"},
    ...
    "10": {"title": "🎯 10: المؤشرات Pointers", "explanation": "متغير يخزن عنوان متغير آخر في الذاكرة.", "example": "int* ptr = &x;", "exercise": "عرف مؤشر ptr من نوع int.", "solution": "int* ptr;"},
    ...
    "13": {"title": "🏗️ 13: الهياكل Structs", "explanation": "تجميع متغيرات مختلفة تحت اسم واحد.", "example": "struct Car { int id; };", "exercise": "عرف struct باسم User.", "solution": "struct User { };"},
...
def py_start(m):
    mk = types.ReplyKeyboardMarkup(resize_keyboard=True).add("📚 قائمة دروس بايثون")
    bot_py.send_message(m.chat.id, "🚀 مرحباً بك في بوت بايثون!", reply_markup=mk)

@bot_py.message_handler(func=lambda m: m.text == "📚 قائمة دروس بايثون")
def py_list(m):
...
def cpp_start(m):
    mk = types.ReplyKeyboardMarkup(resize_keyboard=True).add("📚 قائمة دروس C++")
    bot_cpp.send_message(m.chat.id, "👋 مرحباً بك في بوت C++!", reply_markup=mk)

@bot_cpp.message_handler(func=lambda m: m.text == "📚 قائمة دروس C++")
def cpp_list(m):
    mk = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=3)
    mk.add(*[types.KeyboardButton(f"الدرس {i}") for i in range(1, 15)])
...
def cpp_call(c):
    ...
    else: bot_cpp.edit_message_text(f"✅ <b>الحل:</b>\n<code>{l['solution']}</code>", c.message.chat.id, c.message.message_id, parse_mode="HTML")

# --- نظام التشغيل الآمن ---
def safe_poll(bot, name):
    while True:
        try:
            bot.remove_webhook()
